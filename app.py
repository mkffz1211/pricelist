...
def find_fixture_url(home_team, away_team):
    """
    Try the '-v-' fixture URL first. If that page doesn't load (non-200),
    automatically fall back to '-vs-'. Returns the first working URL, or
    None if neither format resolves.
    """
    for connector in ("v", "vs"):
        candidate = build_fixture_url_with_connector(home_team, away_team, connector)
        try:
...
def fetch_raw_prices(fixture_url, qty, seated_together, category_map):
    ...
    raw = {}
    for cat_id in ids:
        raw[cat_id] = get_cheapest_price(fixture_url, qty, seated_together, [cat_id])
    return raw
...
NAV = """
<div class="max-w-3xl mx-auto px-4 pt-6 flex items-center justify-between text-sm">
  <div class="flex gap-4">
    <a href="/pricelist" class="{price_active} px-3 py-1.5 rounded-full font-medium">Full Price List</a>
    <a href="/baseprice" class="{baseprice_active} px-3 py-1.5 rounded-full font-medium">Base Price</a>
    <a href="/message" class="{message_active} px-3 py-1.5 rounded-full font-medium">Customer Message Final</a>
  </div>
  <form method="POST" action="/reset">
    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-full font-medium transition">Reset</button>
  </form>
</div>
"""
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
  <div>
    <label class="block text-sm font-semibold text-gray-700 mb-1">Home Team</label>
    <select name="home_team" required class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
  ...
  <div>
    <label class="block text-sm font-semibold text-gray-700 mb-1">Away Team</label>
    <select name="away_team" required class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
...
<div class="mt-4">
  <label class="block text-sm font-semibold text-gray-700 mb-1">
    Custom URL override <span class="text-gray-400 font-normal">(optional — use if the auto-generated link doesn't match the site)</span>
  </label>
  <input type="text" name="custom_url" value="{{ custom_url or '' }}" placeholder="https://www.livefootballtickets.com/us/fixtures/...html"
...
<html>
<head>
    <title>Full Price List</title>
    """ + BASE_STYLE + """
</head>
<body class="bg-gray-50 min-h-screen">
    """ + NAV.replace("{price_active}", "bg-emerald-600 text-white").replace("{baseprice_active}", "text-gray-600 hover:bg-gray-200").replace("{message_active}", "text-gray-600 hover:bg-gray-200") + """

    <div class="max-w-3xl mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-1">Full Price List</h1>
        <p class="text-gray-500 text-sm mb-6">Shows every real stand/section and its cheapest current price. Skips junior-only pricing.</p>

        <form method="POST" class="card p-6 mb-6">
            """ + TEAM_SELECT_FIELDS + """
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                ...
                <div class="flex items-end pb-2.5">
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700">
                        <input type="checkbox" name="seatedTogether" {% if seated_together %}checked{% endif %} class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500">
                        Seated together
                    </label>
                </div>
            </div>

            <button type="submit" class="mt-5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition">
                Get Full Price List
            </button>
        </form>

        {% if error %}
            <div class="bg-red-50 text-red-700 text-sm px-4 py-3 rounded-lg mb-6">{{ error }}</div>
        {% endif %}

        {% if fixture_url %}
            <p class="text-xs text-gray-400 mb-4 break-all">Fixture: {{ fixture_url }}</p>
        {% endif %}
        ...
        <div class="card overflow-hidden">
            <table class="w-full text-sm">
                <thead class="bg-gray-100 text-gray-600 text-xs uppercase tracking-wide">
                    <tr>
                        <th class="text-left px-4 py-3">Stand / Section</th>
                        <th class="text-right px-4 py-3">Price</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    {% for r in results %}
                    <tr class="{{ 'bg-emerald-50' if loop.first and r.price != 'N/A' else '' }}">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ r.name }}</td>
                        <td class="px-4 py-3 text-right font-bold {{ 'text-emerald-600' if loop.first and r.price != 'N/A' else 'text-gray-800' }}">
                            {% if r.price == 'N/A' %}
                                <span class="text-gray-400 font-normal text-xs">N/A</span>
                            {% else %}
                                {{ r.currency }} {{ r.price }}
                            {% endif %}
                        </td>
...
<body class="bg-gray-50 min-h-screen">
    """ + NAV.replace("{price_active}", "text-gray-600 hover:bg-gray-200").replace("{baseprice_active}", "bg-emerald-600 text-white").replace("{message_active}", "text-gray-600 hover:bg-gray-200") + """

    <div class="max-w-3xl mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-1">Base Price</h1>
        <p class="text-gray-500 text-sm mb-6">Raw D / D+ / C / B / A / A+ / VIP prices, exactly as fetched (no rounding).</p>
        ...
        if fixture_error:
            ...
            try:
                ...
                results = [
                    ...
                    for label, amount, currency in final
                ]
...
<body class="bg-gray-50 min-h-screen">
    """ + NAV.replace("{price_active}", "text-gray-600 hover:bg-gray-200").replace("{baseprice_active}", "text-gray-600 hover:bg-gray-200").replace("{message_active}", "bg-emerald-600 text-white") + """

    <div class="max-w-3xl mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-1">Customer Message Final</h1>
        <p class="text-gray-500 text-sm mb-6">Ready-to-send price list message, in GBP, rounded down to the nearest £5.</p>
        ...
        {% if message_text %}
        <div class="rounded-xl overflow-hidden shadow-lg max-w-sm mx-auto">
            <div class="wa-header text-white px-4 py-3 flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-emerald-300 flex items-center justify-center text-emerald-800 font-bold text-sm">
            ...
            <div class="wa-bg px-4 py-6">
                <div class="bubble px-3 py-2 max-w-[90%] ml-auto">
                    <p class="text-sm text-gray-900 whitespace-pre-line leading-relaxed">{{ message_text }}</p>
                    <div class="flex justify-end items-center gap-1 mt-1">
                        <span class="text-[10px] text-gray-500">{{ current_time }}</span>
            ...
            <div class="bg-gray-100 px-4 py-3 flex items-center gap-3">
                <div class="flex-1 bg-white rounded-full px-4 py-2 text-sm text-gray-400">Type a message</div>
            </div>
        </div>

        <div class="max-w-sm mx-auto mt-4">
            <button id="copyBtn" onclick="copyMessage()"
                class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-3 rounded-lg text-sm transition flex items-center justify-center gap-2">
...
def message():
    ...
    if attempted:
        if fixture_error:
            ...
            try:
                if session.get("meta_fixture_url") == fixture_url and "meta_stadium" in session:
                    stadium = session["meta_stadium"]
                    match_date = session["meta_match_date"]
                else:
                ...
                for label, amount, currency in final:
                    if amount is not None:
                        ...
                        rounded = None
                    price_str = f"£{rounded:g}" if rounded is not None else "N/A"
